...
} = require("../emergencyContact/emergencyContact.service");
...
const SurvivorController = {
  ...
  getAll: async (req, res) => {
    try {
      ...
      if (req.query.gender) {
        ...
        if (gender.length > 25) {
          return res.validationErrors([
            "El parámetro gender no debe exceder 25 caracteres",
          ]);
        }
      ...
      if (req.query.headquarterId) {
        const headquarterId = ValidationRules.parseIdParam(
          String(req.query.headquarterId)
        );
        if (!headquarterId) {
          return res.validationErrors([
            "El parámetro headquarterId debe ser numérico",
          ]);
        }
      ...
      if (req.query.ageMin) {
        ...
        if (isNaN(ageMin) || ageMin < 0 || ageMin > 150) {
          return res.validationErrors([
            "El parámetro ageMin debe ser un número entre 0 y 150",
          ]);
        }
      ...
      if (req.query.ageMax) {
        ...
        if (isNaN(ageMax) || ageMax < 0 || ageMax > 150) {
          return res.validationErrors([
            "El parámetro ageMax debe ser un número entre 0 y 150",
          ]);
        }
  ...
  getById: async (req, res) => {
    ...
    try {
      ...
      if (!idNum)
        return res.validationErrors(["El parámetro id debe ser numérico"]);
  ...
  create: async (req, res) => {
      ...
      normalized.cancers = normalized.cancers.map((c) => ({
        ...
        stage:
          typeof c.stage === "string"
            ? c.stage.trim().replace(/\s+/g, " ")
            : c.stage,
      }));
    }

    const validation = EntityValidators.survivor(normalized, {
      partial: false,
    });
    ...
    try {
      ...
      if (normalized.idHeadquarter) {
        const headquarter = await HeadquarterService.findById(
          normalized.idHeadquarter
        );
        if (!headquarter) {
      ...
      if (
        !normalized.cancers ||
        !Array.isArray(normalized.cancers) ||
        normalized.cancers.length === 0
      ) {
        ...
        for (let i = 0; i < normalized.cancers.length; i++) {
          ...
          if (!cancer.idCancer || typeof cancer.idCancer !== "number") {
            errors.push(
              `Cáncer en posición ${
                i + 1
              }: idCancer es requerido y debe ser un número`
            );
          } else {
            ...
            if (!cancerExists) {
              errors.push(
                `Cáncer con ID ${cancer.idCancer}: El tipo de cáncer no existe`
              );
            } else if (cancerExists.status !== "active") {
              errors.push(
                `Cáncer con ID ${cancer.idCancer} ("${cancerExists.cancerName}"): El tipo de cáncer no está activo`
              );
            }
          }
          const stageVal = cancer.stage || "";
          if (typeof stageVal !== "string" || stageVal.trim() === "") {
            const cancerId = cancer.idCancer
              ? `ID ${cancer.idCancer}`
              : `posición ${i + 1}`;
            errors.push(
              `Cáncer ${cancerId}: stage (etapa) es requerido y debe ser texto`
            );
          } else {
            // normalize stage spacing
            cancer.stage =
              typeof stageVal === "string"
                ? stageVal.trim().replace(/\s+/g, " ")
                : stageVal;
      ...
      if (normalized.phone) {
        if (
          ...
          typeof normalized.phone !== "number"
        ) {
      ...
      if (
        ...
        !Array.isArray(normalized.emergencyContacts)
      ) {
        errors.push(
          "emergencyContacts debe ser un array de objetos con idEmergencyContact y relationshipType"
        );
        ...
        for (let i = 0; i < data.emergencyContacts.length; i++) {
          ...
          if (typeof contactData === "number") {
            errors.push(
              `Contacto de emergencia ${
                i + 1
              }: Debe proporcionar un objeto con idEmergencyContact y relationshipType`
            );
            continue;
            ...
            if (!idContact || typeof idContact !== "number") {
              errors.push(
                `Contacto de emergencia ${
                  i + 1
                }: idEmergencyContact es requerido y debe ser un número`
              );
              continue;
            }
            ...
            if (
              ...
              errors.push(
                `Contacto de emergencia ${
                  i + 1
                }: relationshipType es requerido y debe ser un texto no vacío`
              );
              continue;
            }
            ...
            if (relationshipType.length > 50) {
              errors.push(
                `Contacto de emergencia ${
                  i + 1
                }: relationshipType no debe exceder 50 caracteres`
              );
              continue;
            }
            ...
            errors.push(
              `Contacto de emergencia ${
                i + 1
              }: Formato inválido, debe ser un objeto con idEmergencyContact y relationshipType`
            );
            continue;
          }
          ...
          if (!contact) {
            errors.push(
              `Contacto de emergencia ${
                i + 1
              }: El contacto con ID ${idContact} no existe`
            );
          } else if (contact.status !== "active") {
            errors.push(
              `Contacto de emergencia ${i + 1}: El contacto "${
                contact.nameEmergencyContact
              }" no está activo`
            );
          }
        }
      ...
      if (normalized.birthday) {
        ...
        if (actualAge < 18) {
          if (
            !normalized.emergencyContacts ||
            normalized.emergencyContacts.length === 0
          ) {
            errors.push(
              "Los supervivientes menores de edad deben tener al menos un contacto de emergencia"
            );
          }
        }
      }
      ...
      if (
        allSurvivors.some(
          (s) => normalizeForCompare(s.documentNumber) === newDocNorm
        )
      ) {
      ...
      if (
        normalized.email &&
        allSurvivors.some((s) => normalizeForCompare(s.email) === newEmailNorm)
      ) {
        duplicateErrors.push(
          "Ya existe un superviviente con ese correo electrónico"
        );
      }
      ...
      if (error.code === "P2000") {
        ...
        return res.validationErrors([
          `El valor proporcionado para ${columnName} es demasiado largo`,
        ]);
      }
  ...
  update: async (req, res) => {
    ...
    if (updateData.__jsonError) {
      return res.validationErrors([
        updateData.__jsonErrorMessage || "Formato de JSON inválido",
      ]);
    }
    ...
    try {
      ...
      if (previousSurvivor.status !== "active") {
        return res.badRequest(
          "No se puede actualizar un superviviente inactivo"
        );
      }

      // Validate headquarter if being updated
      if (
        updateData.idHeadquarter !== undefined &&
        updateData.idHeadquarter !== null
      ) {
        ...
        if (isNaN(headquarterId) || headquarterId <= 0) {
          return res.validationErrors([
            "El ID de la sede debe ser un número entero positivo",
          ]);
        }
        ...
        if (!headquarter) {
          return res.validationErrors([
            `La sede con ID ${headquarterId} no existe`,
          ]);
        }
        if (headquarter.status !== "active") {
          return res.validationErrors([
            `La sede "${headquarter.name}" no está activa`,
          ]);
        }
      }
      ...
      const attemptedRelations = relationFields.filter(
      ...
      if (payload.documentNumber) {
        ...
        const conflict = allSurvivors.find(
          (s) =>
            s.idSurvivor !== previousSurvivor.idSurvivor &&
            normalizeForCompare(s.documentNumber) === docNorm
        );
        if (conflict)
          return res.validationErrors([
            "Ya existe otro superviviente con ese número de documento",
          ]);
      }
      if (payload.email) {
        ...
        const conflict = allSurvivors.find(
          (s) =>
            s.idSurvivor !== previousSurvivor.idSurvivor &&
            normalizeForCompare(s.email) === emailNorm
        );
        if (conflict)
          return res.validationErrors([
            "Ya existe otro superviviente con ese correo electrónico",
          ]);
      }

      const updatedSurvivor = await SurvivorService.update(
        Number(idNum),
        payload
      );
      ...
      if (onlyStatusChange) {
        ...
        // Build complete survivor data for previous version
        const previousData = {
          "ID Superviviente": previousSurvivor.idSurvivor,
          "Sede": formatValue(previousSurvivor.idHeadquarter),
          "Nombre del Superviviente": formatValue(previousSurvivor.survivorName),
          "Número de Documento": formatValue(previousSurvivor.documentNumber),
          "País": formatValue(previousSurvivor.country),
          "Fecha de Nacimiento": formatDate(previousSurvivor.birthday),
          "Correo Electrónico": formatValue(previousSurvivor.email),
          "Residencia": formatValue(previousSurvivor.residence),
          "Género": formatValue(previousSurvivor.genre),
          "Condición Laboral": formatValue(previousSurvivor.workingCondition),
          "CONAPDIS": formatValue(previousSurvivor.CONAPDIS),
          "IMAS": formatValue(previousSurvivor.IMAS),
          "Estado Expediente Físico": formatValue(previousSurvivor.physicalFileStatus),
          "Expediente Médico": formatValue(previousSurvivor.medicalRecord),
          "Fecha Hogar SINRUBE": formatDate(previousSurvivor.dateHomeSINRUBE),
          "Banco de Alimentos": formatValue(previousSurvivor.foodBank),
          "Estudio Socioeconómico": formatValue(previousSurvivor.socioEconomicStudy),
          "Notas": formatValue(previousSurvivor.notes),
          "Estado": formatValue(previousSurvivor.status),
        };

        // Build complete survivor data for updated version
        const updatedData = {
          "ID Superviviente": updatedSurvivor.idSurvivor,
          "Sede": formatValue(updatedSurvivor.idHeadquarter),
          "Nombre del Superviviente": formatValue(updatedSurvivor.survivorName),
          "Número de Documento": formatValue(updatedSurvivor.documentNumber),
          "País": formatValue(updatedSurvivor.country),
          "Fecha de Nacimiento": formatDate(updatedSurvivor.birthday),
          "Correo Electrónico": formatValue(updatedSurvivor.email),
          "Residencia": formatValue(updatedSurvivor.residence),
          "Género": formatValue(updatedSurvivor.genre),
          "Condición Laboral": formatValue(updatedSurvivor.workingCondition),
          "CONAPDIS": formatValue(updatedSurvivor.CONAPDIS),
          "IMAS": formatValue(updatedSurvivor.IMAS),
          "Estado Expediente Físico": formatValue(updatedSurvivor.physicalFileStatus),
          "Expediente Médico": formatValue(updatedSurvivor.medicalRecord),
          "Fecha Hogar SINRUBE": formatDate(updatedSurvivor.dateHomeSINRUBE),
          "Banco de Alimentos": formatValue(updatedSurvivor.foodBank),
          "Estudio Socioeconómico": formatValue(updatedSurvivor.socioEconomicStudy),
          "Notas": formatValue(updatedSurvivor.notes),
          "Estado": formatValue(updatedSurvivor.status),
        };

        // Convert to JSON strings for comparison and logging
        const previousJSON = JSON.stringify(previousData, null, 2);
      ...
      if (process.env.NODE_ENV !== "production") {
        return res.error(
          `Error al actualizar el superviviente: ${error.message}`
        );
      }
  ...
  delete: async (req, res) => {
    ...
    try {
      ...
      if (process.env.NODE_ENV !== "production") {
        return res.error(
          `Error al cambiar el estado del superviviente: ${error.message}`
        );
      }
  ...
  reactivate: async (req, res) => {
    ...
    try {
      ...
      if (process.env.NODE_ENV !== "production") {
        return res.error(
          `Error al reactivar el superviviente: ${error.message}`
        );
      }
